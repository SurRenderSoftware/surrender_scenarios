...
def test_render(with_pytest: bool, config, s, script_dir):
  ...
  #--[Star map]------------------------
  s.setBackground(f"{os.path.abspath(os.path.dirname(__file__))}/starMap_example.txt")
  ...
  if with_pytest:
    from surrender_test.check import check_img_L2
    check_img_L2(config, image, f"{script_dir}/../../surrender-nonreg-test/user_manual/control/SCR07_ref.tif")
  else:
